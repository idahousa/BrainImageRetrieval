...
class UNets:
    ...
    def build(self):
        ...
        return model
"""Implementation for a nested UNet networks"""
class NestedUnets:
    cv_debug = False
    def __init__(self,
                 i_net_name        = 'unet',# switch between 'unet', and 'unetplus'
                 i_shortcut_mani   = True,
                 i_block_name      = 'conv',  # 'conv','sk','atrous','gatrous'
                 i_img_height      = 256,
                 i_img_width       = 256,
    ...
    def build(self):
        ...
        """Reference: Attention enriched deep learning model for breast tumor segmentation in ultrasound images"""
        """Fusion the input image and salient map multiple times using two inputs: input image, and salient maps"""
    ...
    def build_single(self):
        """Fusion the input image and salient map one time at the begining of the deep network"""
        inputs = tf.keras.layers.Input(shape = self.input_shape)
    ...
    def build_multiple(self):
        """Check the Figure 1 of the reference paper: """
        """Reference: Attention enriched deep learning model for breast tumor segmentation in ultrasound images"""
        """Fusion of input image and salient-map multiple times according to the depth of the deep-network"""
        inputs = tf.keras.layers.Input(shape = self.input_shape)
        if self.model is None:
            shallow_model = UNets(i_net_name     = self.net_name,
                                  i_shortcut_mani= self.shortcut_mani,
