...
from libs.networks.losses import SegNetLosses
from libs.networks.unets import UNets,NestedUnets
from libs.networks.callbacks import CustomCallback
from libs.networks.lrs import LearningRateScheduler
from libs.networks.segnet_metrics import SegMetrics,SegMetrics3D
"""=====================================================================================================================
...
class ImageSegNets:
    ...
    """Init network architecture. Check code for more details"""
    def init_network(self):
        if self.net_id==0:#Conventional UNet
            ...
            return UNets(i_net_name        = 'unetplus',
                         ...
                         i_filters         = (32,64,128,256,512),
                         i_supervision     = False).build()
        elif self.net_id==2:#Shallow Unet
            return UNets(i_net_name      = 'unet',
                         i_shortcut_mani = False,
                         i_block_name    = 'conv',
                         i_img_height    = self.input_shape[0],
                         ...
                         i_filters       = (32, 64, 128),
                         i_supervision   = False).build()
        elif self.net_id==3:#Shallow UNetPlus
            return UNets(i_net_name        = 'unetplus',
                         i_shortcut_mani   = False,
                         i_block_name      = 'conv',
                         i_img_height      = self.input_shape[0],
                         ...
                         i_filters         = (32,64,128),
                         i_supervision     = False).build()
        elif self.net_id==4:#Nesetd Shallow-Deep UNet
            shallow_network = ImageSegNets(i_net_id      = 2,
        ...
        elif self.net_id==5:#Nested Shallow-Deep UNetPlus
            shallow_network = ImageSegNets(i_net_id      = 3,
                                           i_save_path   = self.save_path,
                                           i_image_shape = self.input_shape,
                                           i_num_classes = 1,
                                           i_continue    = False).model
            return NestedUnets(i_net_name                ='unetplus',
                               ...
                               i_supervision=False,
                               model=shallow_network).build_multiple()
        else:#Conventional UNet network
            return UNets(i_net_name      = 'unet',
                         i_shortcut_mani = False,
                         i_block_name    = 'conv',
                         i_img_height    = self.input_shape[0],
    ...
    def eval(self,i_db=None,i_debug=False):
        ...
        for index, element in enumerate(i_db):
            ...
            for pred_index,pred in enumerate(preds):
                ...
                current_mask = CSDataset.scale_mask(i_mask=current_mask, i_tsize=self.mask_shape)
...
if __name__ == '__main__':
    ...
    tr_db, va_db = CSDataset.get_sample_db(i_tsize=(256, 256), i_num_train_samples=1000, i_num_val_samples=100)
